...
namespace uTools
{
    public abstract class Tweener : MonoBehaviour
    {
        ...
        public bool UseReverseCurve = false;
        public AnimationCurve animationCurveReverse;
        ...
        float Evaluate(float factor)
        {
            if (UseReverseCurve)
            {
                return mAmountPerDelta > 0 ? animationCurve.Evaluate(factor) : animationCurveReverse.Evaluate(factor);
            }
            else
            {
                return animationCurve.Evaluate(factor);
            }
        }
        /// <summary>
        ...
        /// <summary>
        /// Manually reset all setting of tweener's that like just created.
        /// </summary>
        ...
        public void Pause()
        {
            mPauseTime = ignoreTimeScale ? Time.unscaledTime : Time.time;
            mPause = true;
        }
        public void Resume()
        {
            mStartTime += (ignoreTimeScale ? Time.unscaledTime : Time.time) - mPauseTime;
            mPauseTime = 0f;
            mPause = false;
        }
